...
### Added — agent stack manager & capability search
The daily-driver layer on top of the marketplace: a cross-tool package-manager for the MCP servers your agent uses. New `src/stack/`, mirroring `src/hubs/` — one `ToolAdapter` per tool (opencode / Claude Code / Cursor / Windsurf) normalizing each config into one `ConfiguredServer` shape.
- **`agora installed`** — unified view of every configured MCP server across all detected tools, grouped by name with transport + per-tool scope.
...
- **`agora capabilities [query]`** — searches the MCP *tools* discovered across your servers ("which of my servers can do X"), reusing the offline BM25 engine. Tool schemas are discovered by `doctor --probe` / `try` and persisted to a local capability cache (`src/stack/capability-cache.ts`). The local slice of capability search.
- **`src/stack/mcp-probe.ts`** — a minimal, standalone MCP stdio client (newline-delimited JSON-RPC `initialize` + `tools/list`) with strict process cleanup; groundwork for catalog-wide capability search.
- **Stack introspection via `agora mcp`** — three read-only MCP tools (`stack_installed`, `stack_doctor`, `stack_capabilities`) let an agent see the user's configured servers, their static health, and the tools they expose — so an agent can reason about the stack it's running in. Mutating config still goes only through the gated CLI (`sync --write --yes`).
...
### Internal
- Repo-wide `prettier --write` pass so the CI `format:check` job is green (it had been failing across ~45 pre-existing files, blocking every PR).
- 1098 pass / 1 skip / 0 fail across 50 test files (was 827 across 36). New tests cover the BM25 index, the curator hardening, the stack adapter layer + doctor, the `agora.toml` parser round-trip, `sync` config-preservation, the MCP stdio probe (against a fake-server fixture), the capability cache, and the new commands.
