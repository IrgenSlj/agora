...
import type { NewsConfig, NewsItem } from '../src/news/types.js';
...
describe('readCache / writeCache', () => {
  test('readCache returns empty array for missing file', () => {
    const items = readCache(join(tmpdir(), 'agora-news-does-not-exist'));
    expect(items).toEqual([]);
  });

  test('writeCache + readCache round-trips', () => {
    const dir = mkdtempSync(join(tmpdir(), 'agora-news-cache-'));
    try {
