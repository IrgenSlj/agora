...
describe('buildUpdatePlan', () => {
  test('builds entries for mixed server states', () => {
    ...
    expect(plan[0]!.server).toBe('pinned-outdated');
    expect(plan[0]!.status).toBe('updatable');
    expect(plan[0]!.pkg).toBe('my-pkg');
    expect(plan[0]!.current).toBe('1.0.0');
    expect(plan[0]!.latest).toBe('2.0.0');
    expect(plan[0]!.message).toContain('my-pkg');
    expect(plan[0]!.message).toContain('1.0.0');
    expect(plan[0]!.message).toContain('2.0.0');

    expect(plan[1]!.server).toBe('pinned-current');
    expect(plan[1]!.status).toBe('up-to-date');
    expect(plan[1]!.message).toContain('(latest)');

    expect(plan[2]!.server).toBe('unpinned');
    expect(plan[2]!.status).toBe('tracks-latest');
    expect(plan[2]!.message).toContain('tracks latest');

    expect(plan[3]!.server).toBe('remote-srv');
    expect(plan[3]!.status).toBe('unknown');
    expect(plan[3]!.message).toContain('not an npm package');
  });
...
describe('agora update command', () => {
  ...
  test('unknown --tool returns exit 1 with error', async () => {
  ...
  test('server name filter with no match returns exit 1', async () => {
    const temp = mkdtempSync(join(tmpdir(), 'agora-update-'));
...
describe('agora update --write', () => {
  test('--write without --yes returns non-zero usageError and writes nothing', async () => {
