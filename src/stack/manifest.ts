...
export interface ManifestEntry {
  ...
  url?: string;
  /**
   * Non-sensitive literal environment values. Portable manifests generated by
   * Agora use `envFrom` instead so host-config credentials never enter
   * agora.toml.
   */
  env?: Record<string, string>;
  /** Target environment key -> local process environment key. */
...
export function opencodeEntryToManifest(entry: OpencodeMcpEntry): ManifestEntry {
  ...
  if (entry.type === 'local') {
    ...
    if (entry.environment && Object.keys(entry.environment).length > 0) {
      result.envFrom = Object.fromEntries(Object.keys(entry.environment).map((key) => [key, key]));
    }
  } else {
...
export function serverToEntry(server: ConfiguredServer): ManifestEntry {
  ...
  if (server.env && Object.keys(server.env).length > 0) {
    entry.envFrom = Object.fromEntries(Object.keys(server.env).map((key) => [key, key]));
  }
...
function serializeSection(
  ...
  for (const name of names) {
    ...
    if (entry.envFrom && Object.keys(entry.envFrom).length > 0) {
      lines.push(`[${section}.${seg}.env_from]`);
      for (const key of Object.keys(entry.envFrom).sort()) {
        lines.push(`${key} = "${escapeString(entry.envFrom[key]!)}"`);
      }
    }
    lines.push('');
  }
...
function parseTableHeader(
  line: string,
  lineNum: number
): { section: string; name: string; subtable?: 'env' | 'env_from' } {
  // Strip [ and ]
  ...
  if (segments.length === 3 && segments[2] === 'env') {
    return { section, name: segments[1], subtable: 'env' };
  }
  if (segments.length === 3 && segments[2] === 'env_from') {
    return { section, name: segments[1], subtable: 'env_from' };
  }
  if (segments.length === 2) {
    return { section, name: segments[1] };
  }
...
export function parseManifest(text: string): StackManifest {
  ...
  let currentSubtable: 'env' | 'env_from' | undefined;
  ...
  for (let idx = 0; idx < lines.length; idx++) {
    ...
    if (line.startsWith('[')) {
      ...
      const { section, name, subtable } = parseTableHeader(line, lineNum);
      if (subtable && section === 'instructions') {
        throw new Error(
          `Line ${lineNum}: instructions entries do not support .env or .env_from sub-tables`
        );
      }
    ...
    if (currentSubtable === 'env') {
      // Any key, string value
      const strVal = parseTomlString(rawVal, lineNum);
      entry.env = entry.env ?? {};
      entry.env[key] = strVal;
    } else if (currentSubtable === 'env_from') {
      const sourceKey = parseTomlString(rawVal, lineNum);
      if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(sourceKey)) {
        throw new Error(`Line ${lineNum}: invalid environment variable name "${sourceKey}"`);
      }
      entry.envFrom = entry.envFrom ?? {};
      entry.envFrom[key] = sourceKey;
    } else {
