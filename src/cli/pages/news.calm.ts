...
export const newsPage: Page = {
  ...
  render(ctx: PageContext): string {
    ...
    const lines: string[] = [];
    lines.push(' ' + style.bold(style.accent('NEWS'))
      + '    ' + style.dim('sources: ') + style.accent(state.source)
    ...
    if (list.length === 0) {
      lines.push('   ' + style.dim('No stories. Try ')
        + style.accent('r') + style.dim(' to refresh, ')
        + style.accent('t') + style.dim(' to change topic, or ')
        + style.accent('1\u20134') + style.dim(' to switch source.'));
    ...
    list.forEach((s, i) => {
      ...
      lines.push(' ' + lead + style.dim(rank + '. ')
        + style.accent(s.source.toUpperCase().padEnd(7))
        + titleColor);
      lines.push('         ' + style.dim(s.host + '   \u00b7   ' + s.ageH + 'h   \u00b7   ')
        + style.accent('\u2191 ' + s.up)
        + style.dim('   \u00b7   score ' + s.score.toFixed(2))
