...
interface MpState {
  cursor: number; detail: boolean;
  query: string; filtering: boolean;
  category: string; sort: SortKey;
}
const state: MpState = {
...
export const marketplacePage: Page = {
  ...
  render(ctx: PageContext): string {
    ...
    const lines: string[] = [];
    const head = ' ' + style.bold(style.accent('MARKETPLACE'));
    const right = style.dim('cat:') + style.accent(state.category)
      + style.dim('  sort:') + style.accent(state.sort)
      + style.dim('  ' + items.length + ' items');
    ...
    if (items.length === 0) {
      lines.push('');
      lines.push(' ' + style.dim('Empty \u2014 no marketplace items match the current filter.'));
      lines.push(' ' + style.dim('  ')
        + style.accent('/') + style.dim(' search   ')
        + style.accent('c') + style.dim(' cycle category'));
    ...
    if (state.detail) {
      ...
      if (it) {
        lines.push(' ' + style.bold(it.name)
          + (it.author ? style.dim(' \u00b7 ' + it.author) : ''));
        lines.push(' ' + style.dim((it.tags ?? []).join(' \u00b7 ')));
        lines.push(' ' + sep('details', width - 2, style));
        lines.push(' ' + (it.description ?? ''));
        lines.push(' '
          + style.accent(fmtCount(it.installs ?? 0)) + style.dim(' installs')
          + (it.stars !== undefined
            ? '  ' + style.accent(fmtCount(it.stars)) + style.dim(' \u2605') : ''));
        lines.push(' ' + sep('actions', width - 2, style));
        lines.push(' '
    ...
    const limit = Math.max(0, height - lines.length);
    ...
    for (let i = 0; i < limit && start + i < items.length; i++) {
      ...
      if (!it) continue;
      const sel = start + i === state.cursor;
      const lead = sel ? rail(style) : noRail();
      const idCell = (sel ? style.bold(it.name) : it.name).padEnd(28);
      const installs = style.accent(fmtCount(it.installs ?? 0).padStart(7));
      const stars = it.stars !== undefined
        ? style.accent(fmtCount(it.stars).padStart(6)) : '       ';
      const tags = style.dim('[' + (it.tags ?? []).slice(0, 3).join(',') + ']');
      const row1 = ' ' + lead + idCell + installs + style.dim(' i ')
        + stars + style.dim(' \u2605  ') + tags;
      lines.push(row1);
      const sub = '   ' + (sel ? '' : ' ')
        + style.dim((it.description ?? '') + (it.author ? '  \u00b7  ' + it.author : ''));
      lines.push(sub);
      lines.push(' ' + style.dim('\u2500'.repeat(Math.max(0, width - 2))));
    }
    return frame(lines, width, height);
