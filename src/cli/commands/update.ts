import { readAllServers, ALL_ADAPTERS, getAdapter } from '../../stack/registry.js';
import { checkOutdated } from '../../outdated.js';
import { collectPackages, buildUpdatePlan, bumpCommand } from '../../update.js';
...
export const commandUpdate: CommandHandler = async (parsed, io, style) => {
  ...
  if (servers.length === 0) {
    ...
    writeLine(
      io.stdout,
      theme.muted('Run `agora installed` to see configured servers, `agora search` to find new ones.')
    );
    return 0;
  }
  ...
  if (doWrite && doYes) {
    ...
    for (const [tool, toolEntries] of byTool) {
      ...
      const result: ToolResult = { tool, location: location?.path ?? null, updated: [], skipped: [] };

      if (location === null) {
        for (const entry of toolEntries) {
          result.skipped.push({ server: entry.server, reason: `no writable location for scope ${scope}` });
        }
