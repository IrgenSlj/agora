(function () {
  ...
  const packages = [
    ...
    },
    {
      ...
      description: "Detailed code review prompt covering security, performance, and maintainability.",
    ...
    },
    {
      ...
      description: "A release-readiness skill for changelogs, smoke tests, and staged rollout notes.",
      author: "agora-labs",
      version: "0.4.0",
      category: "skill",
  ...
  const baseDiscussions = [
    ...
    },
    {
      ...
      content: "I put together a skill that turns PR history into a release checklist and rollback note.",
      category: "showcase",
      replies: 12,
      stars: 37,
    ...
    },
    {
      ...
      author: "agora-community",
      content: "Could the marketplace surface compatibility, docs coverage, and maintainer response signals?",
      category: "idea",
      replies: 18,
      stars: 52,
  ...
  function renderOverview() {
    ...
    els.metricGrid.innerHTML = [
      metric("Packages", String(packages.length), `${packages.filter((item) => item.status === "Ready").length} ready`),
  ...
  function renderMarketplace() {
    ...
    els.packageDetail.innerHTML = selected ? packageDetail(selected) : emptyState("Nothing selected", "Package details will appear here.");
  ...
  function renderWorkflows() {
    ...
    els.workflowDetail.innerHTML = selected ? workflowDetail(selected) : emptyState("Nothing selected", "Workflow details will appear here.");
    bindDetailActions(els.workflowDetail);
  }

  function renderCommunity() {
    const items = state.discussions
      .filter((item) => state.discussionCategory === "all" || item.category === state.discussionCategory)
      .filter((item) => matchesQuery(item, state.query))
      .sort((a, b) => Number(b.pinned) - Number(a.pinned) || b.stars - a.stars);

    els.discussionList.innerHTML = items.length
      ? items.map(discussionCard).join("")
      : emptyState("No discussions found", "The current filters have no matching discussion records.");

    els.discussionList.querySelectorAll("[data-toggle-reviewed]").forEach((button) => {
      button.addEventListener("click", () => updateDiscussion(button.dataset.toggleReviewed, { toggleReviewed: true }));
    });

    els.discussionList.querySelectorAll("[data-toggle-pinned]").forEach((button) => {
      button.addEventListener("click", () => updateDiscussion(button.dataset.togglePinned, { togglePinned: true }));
    });
  ...
  function renderInstall() {
    ...
    els.installCommands.innerHTML = commands.length ? commands.join("") : "<code>No npm packages selected.</code>";
  }
  ...
  function matchesQuery(item, query) {
    ...
    const haystack = [item.name, item.title, item.description, item.content, item.author, ...(item.tags || [])]
      .filter(Boolean)
      .join(" ")
      .toLowerCase();
  ...
  function shortName(name) {
    return name.replace("@modelcontextprotocol/server-", "").replace("@", "").replace("Comprehensive ", "");
  }
