---
name: agora
description: Operate the agora CLI (npm agora-hub) — the local-first system manager for an agentic stack of MCP servers, skills, and instruction files. Use this skill whenever a task needs to search or browse MCP servers, inspect or reconcile a user's MCP configuration across harnesses (OpenCode, Claude Code, Cursor, Windsurf), or safely acquire a capability the current agent is missing. Works in any harness with the agora binary on PATH, or via `npx -y agora-hub` when it isn't installed.
license: MIT
---

# Operating Agora

Agora is a package manager for an agentic stack, not a chat assistant — every command is
...
3. **Read the exit code, not just stdout.** Agora's exit codes are part of its contract — a `2` or
   `3` means "stop and look," not "something silently succeeded."
...
| `1` | Usage or runtime error | all commands |
| `2` | Plan has pending changes (nothing written yet), or the trust gate warned and needs explicit acceptance | `plan`, `acquire`, `scan` |
| `3` | Trust gate hard-failed — nothing was written | `acquire`, `scan`, `plan --from`, `apply --from`, `sync --from` |

A few commands intentionally don't use the full range: `agora doctor` is informational (exit `0`
unless `--strict` is passed, then `1` on any server error) and `agora integrate` is `0`/`1` only
(there's no trust gate to warn or fail — it just writes a launcher entry).
...
```bash
agora plan   [--tool <id>] [--scope project|user] [--prune] [--json]   # read-only diff, exit 2 if changes pending
...
exits `3` with nothing written, no exceptions.
